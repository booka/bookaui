package net.boklab.tools.server.rest;
 ...
 * <p/>
 * Patched to skip "Transfer-Encoding: chunked" headers, and avoid double
 * slashes in proxied URL's.
 */
public class ProxyServlet extends HttpServlet {
	...
	if (intProxyResponseCode >= HttpServletResponse.SC_MULTIPLE_CHOICES /* 300 */
	    ...
	    if (followRedirects) {
		httpServletResponse.sendRedirect(stringLocation.replace(getProxyHostAndPort() + this.getProxyPath(),
			stringMyHostName));
		return;
	    }
	...
	if (contentType.startsWith("text/x-gwt-rpc")) {
	    ...
	    String serverUrl = stringProxyHost + ((intProxyPort != 80) ? ":" + intProxyPort : "") + stringPrefixPath;
	...
	try {
		...
		if (fileItemCurrent.isFormField()) {
		    ...
		    FilePart filePart = new FilePart(fileItemCurrent.getFieldName(), // The
			    ...
			    new ByteArrayPartSource(fileItemCurrent.getName(), // The
				    ...
				    // file
				    // name
				    fileItemCurrent.get() // The uploaded file
			    // contents
			    ));
		    // Add the part to the list
	    ...
	    postMethodProxyRequest.setRequestHeader(STRING_CONTENT_TYPE_HEADER_NAME, multipartRequestEntity
		    .getContentType());
